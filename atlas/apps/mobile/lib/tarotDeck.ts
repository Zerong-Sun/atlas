import { normalizeTarotCardName } from "@atlas/engines/tarot";
...
const deckByName = new Map(TAROT_DECK.map((card) => [normalizeTarotCardName(card.name), card]));

export function resolveTarotCard(name: string): TarotCard {
  return deckByName.get(normalizeTarotCardName(name)) ?? TAROT_DECK[0]!;
}

export function getTarotCardImageUri(name: string): string {
  return resolveTarotCard(name).image;
}
