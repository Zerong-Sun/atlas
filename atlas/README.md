...
在终端按 `i`（iOS 模拟器）、`a`（Android）或 Expo Go 扫码。请使用 `npm run dev:mobile` 或先 `cd apps/mobile`；不要在 `atlas/` 目录根直接运行 `npx expo start`。详见 [apps/mobile/README.md](apps/mobile/README.md)。

> 请勿在 `atlas/` 目录根运行 `npx expo start`——会误把 monorepo 根目录当作 Expo 项目，报 `Unable to resolve ../../App`。
...
`main` 分支 push / PR 触发 [`.github/workflows/ci.yml`](../.github/workflows/ci.yml)（在本目录 `atlas/` 下执行 `npm install` 与脚本）：
...
Web 推荐 [Vercel](apps/web/README.md#vercel推荐)。本目录内 `vercel.json` 指定构建输出 `apps/web/dist`；Vercel 项目 **Root Directory** 须为 `atlas`。Supabase 迁移与 Functions 部署步骤见 [apps/web/README.md](apps/web/README.md#supabase-云端配置)。
