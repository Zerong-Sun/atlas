...
> **权威清单已改为自动生成。** 运行 `node scripts/audit_art.mjs --write`，
> 结果写进 `ART_TODO_GENERATED.md`——那份直接从数据表扫出来，不会与游戏实际引用脱节。
>
> 本文保留为**风格与批次说明**：写清每一类图要画成什么样、什么尺寸、先画哪一批。
> 具体缺哪些名字，看生成的那份。

本文件此前是唯一的缺口总表；`ART_TODO_MAP.md` 只覆盖地图与交通，已并入本文。
