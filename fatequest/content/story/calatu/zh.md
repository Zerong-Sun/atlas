---
unit: calatu
lang: zh
source: en
source_rev: be552e8e7919
status: translated
translator: 人工校译
notes: >
  行纪腔，非现代白话。「你须知道」是 Yule 的招牌句式，中译须保留；
  Christendom 作「基督教国」不作「西方」；数字关系照搬不改写。
  地域名词用音译+源语对照（tamghā、bājgāh、masjid 等）。

stamps:
  city.calatu.name: 4b67890dc2d1
  ev.calatu.entry.choice.market: dff8a8e30f89
  ev.calatu.entry.choice.pass: c1ebd610da70
  ev.calatu.entry.choice.shrine: 42854d684489
  ev.calatu.entry.body: 0f5c1fa80e32
  ev.calatu.entry.title: 898cc5b23dea
  ev.calatu.a.title: 74198299e7d3
  ev.calatu.a.body: 4ddf7481ffd9
  ev.calatu.a.choice.walk_walls: 239d4502a6ff
  ev.calatu.a.choice.buy_pepper: eea26e7481dd
  ev.calatu.a.choice.count_ships: e36d25343270
  ev.calatu.b.title: c01834800290
  ev.calatu.b.body: 9d977b6a1842
  ev.calatu.b.choice.hear_refuge: 47560044aaca
  ev.calatu.b.choice.buy_dates: c89c77a4eed5
  ev.calatu.b.choice.ask_inland: af1e60e62e04
  ev.ev_calatu_entry.choice_1_result: 563096f32008
  ev.ev_calatu_entry.choice_2_result: 7b13422a348e
  ev.ev_calatu_entry.choice_3_result: fc073e1c0ef1
  ev.ev_calatu_a.choice_1_result: 245f468b9773
  ev.ev_calatu_a.choice_2_result: 6c58f117d268
  ev.ev_calatu_a.choice_3_result: ac5676aa547c
  ev.ev_calatu_b.choice_1_result: 1cb3913a0b8a
  ev.ev_calatu_b.choice_2_result: 71da337e7649
  ev.ev_calatu_b.choice_3_result: 4af5df6f88ad
  ev.ev_calatu_a_followup.title: 092638ab2a0d
  ev.ev_calatu_a_followup.body: f214b45803c0
  ev.ev_calatu_a_followup.choice_1: 8409ba4cfeb8
  ev.ev_calatu_a_followup.choice_1_result: 987eb071f40b
  ev.ev_calatu_a_followup.choice_2: db79d2f86d83
  ev.ev_calatu_a_followup.choice_2_result: 1ee4714336ee
  ev.ev_calatu_a_followup.choice_3: efe34f2c8817
  ev.ev_calatu_a_followup.choice_3_result: 7f7d209d472c
---

## city.calatu.name

哈剌图（盖勒哈特）

## ev.calatu.entry.choice.market

穿过 bāzār（集市）

## ev.calatu.entry.choice.pass

匆匆穿过 darvāzā（城门）

## ev.calatu.entry.choice.shrine

去 mandir（神庙）看看

## ev.calatu.entry.body

哈剌图乃大城，在同名之湾内。城尊贵，距佐法尔向西北约六百英里，临海。民为撒拉逊人，
臣服忽鲁谟斯。凡忽鲁谟斯之Melic与更强之君开战，便退避此城，盖其地势与城防皆极固。
此地不产谷，皆自外输入；凡来之商船皆携若干。港极大极好，印度载货之船往来极众；自此城，
香料与别货分销内地诸城。又自此输出许多良阿拉伯马至印度。如我先言，自此与诸城每年输印之马，
数目惊人。港岸bāzār（集市）香料成堆。

## ev.calatu.entry.title

记哈剌图湾与其城

## ev.calatu.a.title

哈剌图湾要塞

## ev.calatu.a.body

哈剌图乃海湾内大城，距佐法尔向西北约六百英里，临海。民为撒拉逊人，臣服忽鲁谟斯；港极大极好，印度船只满载香料与别货，锚地拥挤。

## ev.calatu.a.choice.walk_walls

沿城垣而行，俯瞰海湾

## ev.calatu.a.choice.buy_pepper

向印度商人买一袋胡椒

## ev.calatu.a.choice.count_ships

立于突堤，数锚泊之船

## ev.calatu.b.title

Melic避战之所

## ev.calatu.b.body

凡忽鲁谟斯之Melic与更强之君开战，便退避此哈剌图城，盖其地势与城防皆极固。此地不产谷，凡来之商船皆携若干。

## ev.calatu.b.choice.hear_refuge

听人述忽鲁谟斯Melic如何以此城为避所

## ev.calatu.b.choice.buy_dates

向运粮之船买枣

## ev.calatu.b.choice.ask_inland

问商队主人内陆道路何往

## ev.ev_calatu_entry.choice_1_result

你匆匆穿过 darvāzā（城门），通往哈剌图（盖勒哈特）的道路，就此展开在舆图上。

## ev.ev_calatu_entry.choice_2_result

你穿过 bāzār（集市），你在路上用去 一日光阴。你把这番见闻记入行纪。你标出了通往哈剌图（盖勒哈特）的路。

## ev.ev_calatu_entry.choice_3_result

你在神庙的灯火间坐了一会儿。离庙时，守庙人已记得你的名字，还在你掌心放了一撮香。通往哈剌图的海路，已标在你的舆图边角。

## ev.ev_calatu_a.choice_1_result

你沿城垣而行，俯瞰海湾，为此费去你 一日路程。通往忽鲁谟斯的道路，就此展开在舆图上。不到一个时辰，这段记录已落笔成文。

## ev.ev_calatu_a.choice_2_result

你向印度商人买一袋胡椒，银钱易手，囊中为之或增或减。货物易手，你的仓廪因之更满。你把这番见闻记入行纪。

## ev.ev_calatu_a.choice_3_result

你立于突堤，数锚泊之船，通往佐法尔的道路，就此展开在舆图上。不到一个时辰，这段记录已落笔成文。气运稍稍偏向于你。

## ev.ev_calatu_b.choice_1_result

你听人述忽鲁谟斯Melic如何以此城为避所，你把这番见闻记入行纪。你标出了通往忽鲁谟斯的路。时运待你，比先前好了几分。

## ev.ev_calatu_b.choice_2_result

你向运粮之船买枣，一注钱款进出，得失皆在掌中。这笔买卖，让你的货舱又添了几分。不到一个时辰，这段记录已落笔成文。

## ev.ev_calatu_b.choice_3_result

你问商队主人内陆道路何往，你标出了通往可马底的路。你标出了通往佐法尔的路。你把这番见闻记入行纪。

## ev.ev_calatu_a_followup.title

哈剌图：海湾要塞的墙巡

## ev.ev_calatu_a_followup.body

在海湾要塞墙上，一位为忽鲁谟斯的马立克点过船只的守将带你沿女墙走。他说哈剌图离外海六百里，然而来自佐法尔的每艘胡椒船与来自可马底的每支内陆商队都须应答这道防波堤——若你想带走的不只是对海湾的一瞥。

## ev.ev_calatu_a_followup.choice_1

按要塞码头价买下胡椒

## ev.ev_calatu_a_followup.choice_1_result

他卖给你封好走海湾的胡椒。价钱里已含码头费。

## ev.ev_calatu_a_followup.choice_2

追问通往可马底的内陆路

## ev.ev_calatu_a_followup.choice_2_result

可马底连同内陆日子已标上舆图。平原之路清晰。

## ev.ev_calatu_a_followup.choice_3

点一日船，听忽鲁谟斯之名

## ev.ev_calatu_a_followup.choice_3_result

你点了一日桅杆。忽鲁谟斯与佐法尔被点名，时运也稍稍偏向你。
