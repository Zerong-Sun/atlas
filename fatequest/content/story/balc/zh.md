---
unit: balc
lang: zh
source: en
source_rev: e07a1e7d91d5
status: translated
translator: 人工校译
notes: >
  行纪腔，非现代白话。「你须知道」是 Yule 的招牌句式，中译须保留；
  Christendom 作「基督教国」不作「西方」；数字关系照搬不改写。
  地域名词用音译+源语对照（tamghā、bājgāh、masjid 等）。

stamps:
  city.balc.name: 79b83c4de8dd
  ev.balc.a.choice.explore: 679acee0db50
  ev.balc.a.choice.fragment: 7e887bd6acb8
  ev.balc.a.choice.listen: ca6c9a2f1d48
  ev.balc.b.choice.lapis: e2b1189ce5c0
  ev.balc.b.choice.ruby: 1e367729c511
  ev.balc.b.choice.talk_traders: 184e42ccf094
  ev.balc.c.choice.chant: ca012a565c29
  ev.balc.c.choice.offering: 5ba46ef3e239
  ev.balc.c.choice.sleep: 63eaaf6d393a
  ev.balc.entry.choice.customs: 5f67b1abbf38
  ev.balc.entry.choice.lore: 5800603dbcaf
  ev.balc.entry.choice.market: b23d98ceae5c
  ev.balc.a.body: ab472b55a3c0
  ev.balc.a.title: 65c829483ee7
  ev.balc.b.body: 440c475344e4
  ev.balc.b.title: fd454524f0d4
  ev.balc.c.body: c78868a08f00
  ev.balc.c.title: c64d043ebe3c
  ev.balc.entry.body: bf58837bab8b
  ev.balc.entry.title: 12d06f446c9b
  ev.ev_balc_entry.choice_1_result: 9e008c35e127
  ev.ev_balc_entry.choice_2_result: 88a5efe5d9b6
  ev.ev_balc_entry.choice_3_result: 782067b1eb1a
  ev.ev_balc_a.choice_1_result: 9e008c35e127
  ev.ev_balc_a.choice_2_result: 88a5efe5d9b6
  ev.ev_balc_a.choice_3_result: 782067b1eb1a
  ev.ev_balc_b.choice_1_result: 9e008c35e127
  ev.ev_balc_b.choice_2_result: 88a5efe5d9b6
  ev.ev_balc_b.choice_3_result: 782067b1eb1a
  ev.ev_balc_c.choice_1_result: 9e008c35e127
  ev.ev_balc_c.choice_2_result: 88a5efe5d9b6
  ev.ev_balc_c.choice_3_result: 782067b1eb1a
  ev.ev_balc_consequence_a.title: 209a66c1ef89
  ev.ev_balc_consequence_a.body: 146f69bcc05f
  ev.ev_balc_consequence_a.choice_1: 4012a4cadc73
  ev.ev_balc_consequence_a.choice_1_result: 9e008c35e127
  ev.ev_balc_consequence_a.choice_2: 2d73ba8cc84a
  ev.ev_balc_consequence_a.choice_2_result: 88a5efe5d9b6
  ev.ev_balc_consequence_a_resolution.title: bd7015098a4d
  ev.ev_balc_consequence_a_resolution.body: 357c2908f927
  ev.ev_balc_consequence_a_resolution.choice_1: 3b4827283f98
  ev.ev_balc_consequence_a_resolution.choice_1_result: 9e008c35e127
  ev.ev_balc_consequence_a_resolution.choice_2: c200bc7d41b1
  ev.ev_balc_consequence_a_resolution.choice_2_result: 88a5efe5d9b6
  ev.ev_balc_consequence_b.title: b6675e80e7b8
  ev.ev_balc_consequence_b.body: 3ab1d5a1f10a
  ev.ev_balc_consequence_b.choice_1: 5624c3fd45af
  ev.ev_balc_consequence_b.choice_1_result: 9e008c35e127
  ev.ev_balc_consequence_b.choice_2: bb0917a651a0
  ev.ev_balc_consequence_b.choice_2_result: 88a5efe5d9b6
  ev.ev_balc_consequence_b_resolution.title: bd7015098a4d
  ev.ev_balc_consequence_b_resolution.body: 827b72f18207
  ev.ev_balc_consequence_b_resolution.choice_1: 3b4827283f98
  ev.ev_balc_consequence_b_resolution.choice_1_result: 9e008c35e127
  ev.ev_balc_consequence_b_resolution.choice_2: c200bc7d41b1
  ev.ev_balc_consequence_b_resolution.choice_2_result: 88a5efe5d9b6
  ev.balc.mentor_closure.title: 4846153e69c7
  ev.balc.mentor_closure.body: 5fd1c1c88d99
  ev.balc.mentor_closure.choice_1: da8d611ab3c7
  ev.balc.mentor_closure.choice_1_result: 9e008c35e127
  ev.balc.mentor_closure.choice_2: 59a447ce21bf
  ev.balc.mentor_closure.choice_2_result: 88a5efe5d9b6
  ev.balc.mentor_closure.choice_3: 630a8fcfe069
  ev.balc.mentor_closure.choice_3_result: 782067b1eb1a
  ev.ev_balc_a_followup.title: 0646cd53a287
  ev.ev_balc_a_followup.body: 863716ce9695
  ev.ev_balc_a_followup.choice_1: 09323f4f5fa1
  ev.ev_balc_a_followup.choice_1_result: 6cc63b8477a4
  ev.ev_balc_a_followup.choice_2: 84a1df6a5747
  ev.ev_balc_a_followup.choice_2_result: 8903b269da44
  ev.ev_balc_a_followup.choice_3: 1e22d1e90a67
  ev.ev_balc_a_followup.choice_3_result: b7c896cfb472
  ev.ev_balc_b_followup.title: f6ea0a80d37a
  ev.ev_balc_b_followup.body: 76de37f22669
  ev.ev_balc_b_followup.choice_1: 425f2d34c7e8
  ev.ev_balc_b_followup.choice_1_result: 89aed740077a
  ev.ev_balc_b_followup.choice_2: 6874e05906ac
  ev.ev_balc_b_followup.choice_2_result: b77fe6d4af20
  ev.ev_balc_b_followup.choice_3: 12bfd4ce2618
  ev.ev_balc_b_followup.choice_3_result: a9b80211e108
  ev.ev_balc_c_followup.title: f5dab986e636
  ev.ev_balc_c_followup.body: bce051c6ffc7
  ev.ev_balc_c_followup.choice_1: c463ae9d3394
  ev.ev_balc_c_followup.choice_1_result: bf89e3c34365
  ev.ev_balc_c_followup.choice_2: e6632769b6f0
  ev.ev_balc_c_followup.choice_2_result: ac0914098178
  ev.ev_balc_c_followup.choice_3: a187fa966375
  ev.ev_balc_c_followup.choice_3_result: 094d6ea45d65
  ev.ev_balc_mentor_followup.title: c66d3e00162a
  ev.ev_balc_mentor_followup.body: d6728efc546c
  ev.ev_balc_mentor_followup.choice_1: d1e13aa012d3
  ev.ev_balc_mentor_followup.choice_1_result: 02f5169a6564
  ev.ev_balc_mentor_followup.choice_2: 092289fce4b4
  ev.ev_balc_mentor_followup.choice_2_result: 086cc562a618
  ev.ev_balc_mentor_followup.choice_3: 4dce0d2c23df
  ev.ev_balc_mentor_followup.choice_3_result: ae02937a15d5
---

## city.balc.name

巴里黑

## ev.balc.a.choice.explore

在亚历山大娶罗克珊娜的地方寻访废墟

## ev.balc.a.choice.fragment

捡一块旧城的雕刻残石

## ev.balc.a.choice.listen

问一个老人这城最盛时是什么样子

## ev.balc.b.choice.lapis

买一块巴达赫尚的青金石原石

## ev.balc.b.choice.ruby

买一颗巴剌红宝石——在威尼斯值一条船

## ev.balc.b.choice.talk_traders

问商队首领路通往何处

## ev.balc.c.choice.chant

在黎明听祆教祭司诵唱

## ev.balc.c.choice.offering

在那盏从未熄灭的火前献香

## ev.balc.c.choice.sleep

在三贤墓旁睡一晚等一个梦

## ev.balc.entry.choice.customs

到 bājgāh（税卡）报关

## ev.balc.entry.choice.lore

打听这座城的来历

## ev.balc.entry.choice.market

穿过大 bāzār（集市）

## ev.balc.a.body

巴里黑昔为诸城之母——大夏之都，亚历山大大帝在此娶粟特的罗克珊娜为妻。城墙所围之地，
广于亚洲任何一城；庙中藏印度、波斯与希腊之宝。如今残破的城垣、坍塌的穹顶与空宫，
讲述着乌浒河以东每座城的同一故事：蒙古骑手来过，无一物直立。然商队仍在此停歇，城自废墟中缓缓重建。
新屋较小，以旧城之石砌成，然集市又活。巴里黑之王向大汗纳贡；青金石、红宝石与土库曼马的贸易，
维系着此城之心。

## ev.balc.a.title

大夏废墟

## ev.balc.b.body

巴里黑的驿栈是丝路上最大者之一——堡垒般的大院，环以可容千驼的厩房，库房堆着丝包与青金石皮囊，
各国商人各有居室。东行十二日可达的巴达哈伤矿中所出青金石，是此城首珍；以原块出售，
换银、丝与香料。山中巴剌斯红宝石之价，在威尼斯可买一条船。土库曼草原的马在内院拍卖。
驿栈终夜不寐；拱廊下火光彻夜，羊肉串与藏红花饭的气味弥漫空中。

## ev.balc.b.title

巴里黑驿栈

## ev.balc.c.body

城外小丘上，古波斯信仰的火庙仍在燃烧——祆教祭司守护一焰，言自琐罗亚斯德之世、先知之前千年从未熄灭。
人称其为三贤墓，并言拜访圣婴基督的三位智者葬于此，三具石棺上犹有金箔残迹。波斯、
印度乃至遥远契丹的朝圣者登丘，献香与银币。白袍祆教祭司于黎明与黄昏诵古颂。人言：
宿于圣陵者将梦见己之死——而他们所记之梦，无一不应。

## ev.balc.c.title

祆教圣陵

## ev.balc.entry.body

巴里黑乃一尊贵大城，然昔日更盛。鞑靼与别国已大加蹂躏毁坏。从前有许多华美宫殿与大理石建筑，
废墟至今犹存。城中人言：亚历山大即在此娶大流士之女为妻。你须知道：此地乃黎凡特鞑靼领主帝国之尽头，
亦为波斯向东与东北方向之极限。如今离开此城，我再告诉你一个叫DOGANA的地方。
既离此城，向东北偏东骑行约十二日，不见人烟；盖人皆避入山中险固之处，以避盗匪与军队之扰。
路上水多，猎物亦丰；亦有狮子。路上无粮可购，此十二日所需，须尽自携带。

## ev.balc.entry.title

巴里黑城

## ev.ev_balc_entry.choice_1_result

这一选择留下了痕迹：巴里黑的人们已把你记在废墟与蓝宝石商路之间。

## ev.ev_balc_entry.choice_2_result

事情尚未完结，但下一段路已经显出：你的名字随废墟与蓝宝石商路先一步传开。

## ev.ev_balc_entry.choice_3_result

你暂且把此事收下。得到的线索仍会指向你在巴里黑的下一步行动。

## ev.ev_balc_a.choice_1_result

这一选择留下了痕迹：巴里黑的人们已把你记在废墟与蓝宝石商路之间。

## ev.ev_balc_a.choice_2_result

事情尚未完结，但下一段路已经显出：你的名字随废墟与蓝宝石商路先一步传开。

## ev.ev_balc_a.choice_3_result

你暂且把此事收下。得到的线索仍会指向你在巴里黑的下一步行动。

## ev.ev_balc_b.choice_1_result

这一选择留下了痕迹：巴里黑的人们已把你记在废墟与蓝宝石商路之间。

## ev.ev_balc_b.choice_2_result

事情尚未完结，但下一段路已经显出：你的名字随废墟与蓝宝石商路先一步传开。

## ev.ev_balc_b.choice_3_result

你暂且把此事收下。得到的线索仍会指向你在巴里黑的下一步行动。

## ev.ev_balc_c.choice_1_result

这一选择留下了痕迹：巴里黑的人们已把你记在废墟与蓝宝石商路之间。

## ev.ev_balc_c.choice_2_result

事情尚未完结，但下一段路已经显出：你的名字随废墟与蓝宝石商路先一步传开。

## ev.ev_balc_c.choice_3_result

你暂且把此事收下。得到的线索仍会指向你在巴里黑的下一步行动。

## ev.ev_balc_consequence_a.title

巴里黑：门上的印记

## ev.ev_balc_consequence_a.body

第一个回答并没有在城门处结束。吏员、脚夫或旅人把你引到一旁，问你打算从巴里黑带走什么。问题背后正是废墟与蓝宝石商路；你须决定如何对待这道征兆。

## ev.ev_balc_consequence_a.choice_1

记下这个名字，追问谁能为它作保

## ev.ev_balc_consequence_a.choice_1_result

这一选择留下了痕迹：巴里黑的人们已把你记在废墟与蓝宝石商路之间。

## ev.ev_balc_consequence_a.choice_2

谢过来人，把自己的打算收在心里

## ev.ev_balc_consequence_a.choice_2_result

事情尚未完结，但下一段路已经显出：你的名字随废墟与蓝宝石商路先一步传开。

## ev.ev_balc_consequence_a_resolution.title

巴里黑：后果显出形状

## ev.ev_balc_consequence_a_resolution.body

你选择带走城门上的名字，便把一次擦肩而过的交谈变成了一桩小小的承诺。它不会替你决定整段旅程，却会让下一个人有理由为你开门。

## ev.ev_balc_consequence_a_resolution.choice_1

在天黑前把这份引荐送到

## ev.ev_balc_consequence_a_resolution.choice_1_result

这一选择留下了痕迹：巴里黑的人们已把你记在废墟与蓝宝石商路之间。

## ev.ev_balc_consequence_a_resolution.choice_2

把这道征兆收进自己的行纪

## ev.ev_balc_consequence_a_resolution.choice_2_result

事情尚未完结，但下一段路已经显出：你的名字随废墟与蓝宝石商路先一步传开。

## ev.ev_balc_consequence_b.title

巴里黑：回答之后的路

## ev.ev_balc_consequence_b.body

回答把一条传闻送上了道路。日落之前，一个熟悉巴里黑日常生计的人又讲起废墟与蓝宝石商路。这说法不如游记宏大，却更适合带在身上。

## ev.ev_balc_consequence_b.choice_1

照着这条实用的建议走

## ev.ev_balc_consequence_b.choice_1_result

这一选择留下了痕迹：巴里黑的人们已把你记在废墟与蓝宝石商路之间。

## ev.ev_balc_consequence_b.choice_2

把这条传闻留给讲述它的人

## ev.ev_balc_consequence_b.choice_2_result

事情尚未完结，但下一段路已经显出：你的名字随废墟与蓝宝石商路先一步传开。

## ev.ev_balc_consequence_b_resolution.title

巴里黑：后果显出形状

## ev.ev_balc_consequence_b_resolution.body

你选择带走道路上的传闻，便把一次擦肩而过的交谈变成了一桩小小的承诺。它不会替你决定整段旅程，却会让下一个人有理由为你开门。

## ev.ev_balc_consequence_b_resolution.choice_1

在天黑前把这份引荐送到

## ev.ev_balc_consequence_b_resolution.choice_1_result

这一选择留下了痕迹：巴里黑的人们已把你记在废墟与蓝宝石商路之间。

## ev.ev_balc_consequence_b_resolution.choice_2

把这道征兆收进自己的行纪

## ev.ev_balc_consequence_b_resolution.choice_2_result

事情尚未完结，但下一段路已经显出：你的名字随废墟与蓝宝石商路先一步传开。

## ev.balc.mentor_closure.title

巴里黑的引路人

## ev.balc.mentor_closure.body

离开巴里黑之前，蓝宝石商路上的老抄写员问你从这里看出了什么。这个教诲不是法术，也不是交易；它只是教你留意道路转弯之后真正有用的细节。

## ev.balc.mentor_closure.choice_1

问这条路将要求你付出什么

## ev.balc.mentor_closure.choice_1_result

这一选择留下了痕迹：巴里黑的人们已把你记在废墟与蓝宝石商路之间。

## ev.balc.mentor_closure.choice_2

留下 一日，听完这段完整的讲述

## ev.balc.mentor_closure.choice_2_result

事情尚未完结，但下一段路已经显出：你的名字随废墟与蓝宝石商路先一步传开。

## ev.balc.mentor_closure.choice_3

谢过引路人，继续上路

## ev.balc.mentor_closure.choice_3_result

你暂且把此事收下。得到的线索仍会指向你在巴里黑的下一步行动。

## ev.ev_balc_a_followup.title

巴里黑：巴克特拉之守

## ev.ev_balc_a_followup.body

你在废宫之间行走时，一位看守此地的老人从歇处醒来，起身迎你。他看护着每一座前来翻拣诸城之母残骸的掘墓人与旅人。他不拦你；只求你先行细听。他说，巴里黑的残石记得的，比蒙古人留下直立之物更多。你若要从巴克特拉带走一块残片，也须带走一分它原本为何物的知识。

## ev.ev_balc_a_followup.choice_1

收下老人从旧墙取下的残石

## ev.ev_balc_a_followup.choice_1_result

老人从一座掠夺者从未触及的壁龛中取出一块雕石，按入你掌中。巴克特拉最后一块完好的残片从此随你同行，老人对旧城的讲述也随之而去。

## ev.ev_balc_a_followup.choice_2

问老人，昔日诸王的青金石从何而来

## ev.ev_balc_a_followup.choice_2_result

老人指向东北，越过群山，指向大夏诸王视为己有的矿场。巴达哈伤之名现在上了你的舆图，你也明白那蓝色之石为何值一个王国。

## ev.ev_balc_a_followup.choice_3

答应在前方的城市为巴克特拉美言

## ev.ev_balc_a_followup.choice_3_result

老人接受你的承诺，别无他求。在巴里黑，听过你许诺的人如今看重你的言语。

## ev.ev_balc_b_followup.title

巴里黑：商队首领的账簿

## ev.ev_balc_b_followup.body

巴里黑的商队首领并不像撒谷粒一样抛撒他们的见识。当其中一位终于肯在拱廊旁的长凳坐下，摊开账册，那一页写满了名字：税卡、水井、牧场，以及其间相隔的日数。他为你逐一念出，再把书转过来，让你看见页边画着的道路。要如何处置这一页，由你自己决定。

## ev.ev_balc_b_followup.choice_1

按账册上写明的价格买一块青金石

## ev.ev_balc_b_followup.choice_1_result

首领按自己的底价卖你一块青金石原石，不还价，也没有花招。石头是真的，价钱公道，拱廊容你带着它离开。

## ev.ev_balc_b_followup.choice_2

循页边所画的道路，向撒马尔罕进发

## ev.ev_balc_b_followup.choice_2_result

去撒马尔罕的路现在清楚地印在你心里：税卡、有水的牧场、两处山口。你不会再盲目地踏上它。

## ev.ev_balc_b_followup.choice_3

等一日，待商旅报告抄毕

## ev.ev_balc_b_followup.choice_3_result

你在拱廊下等了一日，由书记抄录当日的商情报告。等待花费时日，但账册上的功课，在你离去后依然随身。

## ev.ev_balc_c_followup.title

巴里黑：圣陵之梦

## ev.ev_balc_c_followup.body

你在巴里黑城外的小丘上醒来，那梦仍清晰如昨，正如祆教祭司所许。守了一夜火的祭司身披白袍，未曾移动。他望着你，不出一问，随即点头，仿佛这梦本在意料之中。要将它当作警告、当作馈赠，还是默默藏在心里，他全留给你自己。

## ev.ev_balc_c_followup.choice_1

请祭司为你的旅途赐福

## ev.ev_balc_c_followup.choice_1_result

祭司用永不熄灭之火的灰烬在你额上作记，念出一句比此城更古老的祝祷。圣陵的一分神圣，从此随你而行。

## ev.ev_balc_c_followup.choice_2

留下 一日，问这梦意味着什么

## ev.ev_balc_c_followup.choice_2_result

你在焚香之间留了一日，听祭司为你解梦。他的回答记入你的行纪，而这答案并不在你意料之中。

## ev.ev_balc_c_followup.choice_3

把这梦收在自己心里，什么也不说

## ev.ev_balc_c_followup.choice_3_result

你默默离开小丘，带着那梦，无人能为你称量它。有些事，本就该独自携带。

## ev.ev_balc_mentor_followup.title

巴里黑：抄写员的临别之赠

## ev.ev_balc_mentor_followup.body

蓝宝石商路上的老抄写员不让你只带走他的讲述。他伸手探入行囊，把他谋生的最后物件铺开：一页残纸、一张旧舆图、一日的耐心。他让你自己挑哪一件最合用，因为一条路的好坏，全看你带什么走上它。

## ev.ev_balc_mentor_followup.choice_1

收下抄写员对前路的笔记

## ev.ev_balc_mentor_followup.choice_1_result

这笔记如其人一般素朴而准确。你在巴里黑抄写台上所学到的，如今已写成文字，在路上随时可以翻阅。

## ev.ev_balc_mentor_followup.choice_2

问青金石究竟经哪条山路而来

## ev.ev_balc_mentor_followup.choice_2_result

抄写员凭记忆画出通往巴达哈伤的山路——比商队路线短一日，也难一日。如今两条路你都知道了。

## ev.ev_balc_mentor_followup.choice_3

留一日，亲手抄录这本行路之书

## ev.ev_balc_mentor_followup.choice_3_result

你在抄写员的长凳上度过一日，学他的手笔与他的路。这花费了时日，但这本书从此有一半是你的。
