---
unit: camadi
lang: zh
source: en
source_rev: 0e14cdb1b114
status: translated
translator: 人工校译
notes: >
  行纪腔，非现代白话。「你须知道」是 Yule 的招牌句式，中译须保留；
  Christendom 作「基督教国」不作「西方」；数字关系照搬不改写。
  地域名词用音译+源语对照（tamghā、bājgāh、masjid 等）。

stamps:
  city.camadi.name: a26c563705df
  ev.camadi.entry.choice.market: dff8a8e30f89
  ev.camadi.entry.choice.pass: 991514c173df
  ev.camadi.entry.choice.shrine: 32b9d6e49020
  ev.camadi.entry.body: bfe1d5b22150
  ev.camadi.entry.title: f6c7e4777628
  ev.camadi.a.title: d8638d334c8b
  ev.camadi.a.body: d26586852472
  ev.camadi.a.choice.walk_ruins: ff3e669f94b2
  ev.camadi.a.choice.buy_dates: 1089192820e0
  ev.camadi.a.choice.ask_raids: f10fc2d8fa5b
  ev.camadi.b.title: a3f24bced225
  ev.camadi.b.body: ce480f33ef6a
  ev.camadi.b.choice.rest_shade: c899b924d556
  ev.camadi.b.choice.ask_calatu: 9bb3921f48a5
  ev.camadi.b.choice.buy_cotton: 69f4ec157aa5
  ev.ev_camadi_entry.choice_1_result: 02bb53203073
  ev.ev_camadi_entry.choice_2_result: 2d0b947b8c36
  ev.ev_camadi_entry.choice_3_result: 81ec513a4811
  ev.ev_camadi_a.choice_1_result: 8295ce9f1b31
  ev.ev_camadi_a.choice_2_result: c8bcb4ccdcf6
  ev.ev_camadi_a.choice_3_result: 34ef67d497e2
  ev.ev_camadi_b.choice_1_result: a21992df3ead
  ev.ev_camadi_b.choice_2_result: 90bce7b6861d
  ev.ev_camadi_b.choice_3_result: 1dee16e04711
  ev.ev_camadi_a_followup.title: 7c5198d4d30a
  ev.ev_camadi_a_followup.body: bca395fdae2a
  ev.ev_camadi_a_followup.choice_1: e5c9dfdb21a1
  ev.ev_camadi_a_followup.choice_1_result: e7d91d07a4ca
  ev.ev_camadi_a_followup.choice_2: 403fe9ca0dd0
  ev.ev_camadi_a_followup.choice_2_result: 33c5a3a3f03a
  ev.ev_camadi_a_followup.choice_3: 7ff3f4d9a362
  ev.ev_camadi_a_followup.choice_3_result: ea714c7d3306
---

## city.camadi.name

可马底

## ev.camadi.entry.choice.market

穿过 bāzār（集市）

## ev.camadi.entry.choice.pass

匆匆穿过 darvāzeh（城门）

## ev.camadi.entry.choice.shrine

去 masjid（清真寺）看看

## ev.camadi.entry.body

下山骑行两日，便至一片大平原，平原起处有城名卡马迪。昔日曾为大而尊贵之地，如今已不足道——鞑靼屡次劫掠。
此平原极热，今入之省名曰REOBARLES。土产有枣、开心果与天堂苹果，以及我冷国所无有之果。
[斑鸠极多，为果所诱，然撒拉逊人从不捕之，以为不洁。]平原上又有一种石鸡，异于他国：
羽色黑白相间，喙与足朱红。走兽亦奇；且先说其牛。

## ev.camadi.entry.title

卡马迪

## ev.camadi.a.title

平原废城

## ev.camadi.a.body

自山中下行二日，至一大平原，平原起处有城名可马底——昔日曾为大而尊贵之地，如今已不足道，盖鞑靼屡次入侵，数度劫掠。

## ev.camadi.a.choice.walk_ruins

行于断垣间，问此城昔日何貌

## ev.camadi.a.choice.buy_dates

向仍存于平原的果园买枣

## ev.camadi.a.choice.ask_raids

听商队人述鞑靼入侵之事

## ev.camadi.b.title

炎热平原

## ev.camadi.b.body

此平原极热，今入之省名曰Reobarles。土产有枣、开心果与天堂苹果，以及我冷国所无之果；自山而下，益觉此城昔日之盛已去。

## ev.camadi.b.choice.rest_shade

于枣树荫下歇一日

## ev.camadi.b.choice.ask_calatu

问何路下抵哈剌图湾

## ev.camadi.b.choice.buy_cotton

向道旁贩者买棉布

## ev.ev_camadi_entry.choice_1_result

你匆匆穿过 darvāzeh（城门），你标出了通往可马底的路。

## ev.ev_camadi_entry.choice_2_result

你穿过 bāzār（集市），为此费去你 1 日路程。不到一个时辰，这段记录已落笔成文。通往可马底的道路，就此展开在舆图上。

## ev.ev_camadi_entry.choice_3_result

你去 masjid（清真寺）看看，此地人谈起你，语气添了几分敬重。你带走此地的一枚小小印记。你标出了通往可马底的路。

## ev.ev_camadi_a.choice_1_result

你行于断垣间，问此城昔日何貌，你在路上用去 1 日光阴。你把这番见闻记入行纪。你标出了通往克尔曼的路。

## ev.ev_camadi_a.choice_2_result

你向仍存于平原的果园买枣，一注钱款进出，得失皆在掌中。这笔买卖，让你的货舱又添了几分。不到一个时辰，这段记录已落笔成文。

## ev.ev_camadi_a.choice_3_result

你听商队人述鞑靼入侵之事，时运待你，比先前好了几分。你标出了通往科比南的路。你把这番见闻记入行纪。

## ev.ev_camadi_b.choice_1_result

你于枣树荫下歇一日，为此费去你 1 日路程。气运稍稍偏向于你。不到一个时辰，这段记录已落笔成文。

## ev.ev_camadi_b.choice_2_result

你问何路下抵哈剌图湾，你标出了通往哈剌图（盖勒哈特）的路。你标出了通往克尔曼的路。你把这番见闻记入行纪。

## ev.ev_camadi_b.choice_3_result

你向道旁贩者买棉布，一注钱款进出，得失皆在掌中。这笔买卖，让你的货舱又添了几分。你的行事之名先你一步传开。

## ev.ev_camadi_a_followup.title

卡玛迪：废墟平原上的长老

## ev.ev_camadi_a_followup.body

在卡玛迪残墙之间，一位记得劫掠岁月的长老坐在枣树荫下，不让你只看一眼就走。他说平原上仍有劫匪未烧毁的路——通往起儿漫，通往科比南，通往海湾。若你要从这废墟上路，他要你先听清日子的尺度。

## ev.ev_camadi_a_followup.choice_1

按长老的定价买下一袋枣子供下一段路

## ev.ev_camadi_a_followup.choice_1_result

长老按公道分量卖给你枣子。这袋果能撑过炎热的下坡，卡玛迪的旧事也随它同行。

## ev.ev_camadi_a_followup.choice_2

请他点名通往卡拉图的内陆路

## ev.ev_camadi_a_followup.choice_2_result

他在尘土上画出通往卡拉图的内陆路：水井、关卡、驿站之间的日子。海湾要塞已出现在你的舆图上。

## ev.ev_camadi_a_followup.choice_3

坐上一天，听卡玛迪如何陷落

## ev.ev_camadi_a_followup.choice_3_result

你在枣树下坐了一日，听他讲平原如何被蹂躏。故事耗费时日，卡玛迪却不再是无名的废墟。
