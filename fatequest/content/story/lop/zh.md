---
unit: lop
lang: zh
source: en
source_rev: 4a02bd88dfdd
status: translated
translator: 人工校译
notes: >
  行纪腔，非现代白话。「你须知道」是 Yule 的招牌句式，中译须保留；
  Christendom 作「基督教国」不作「西方」；数字关系照搬不改写。
  地域名词用音译+源语对照（tamghā、bājgāh、masjid 等）。

stamps:
  city.lop.name: 7d248d33358b
  ev.lop.entry.choice.ask: 384ef49af533
  ev.lop.entry.choice.rest: 99cc9386e2da
  ev.lop.entry.title: 147de5f8d9b6
  ev.lop.bazaar.body: 2e44378eff95
  ev.lop.bazaar.choice.ask_prices: d01750e1e10a
  ev.lop.bazaar.choice.buy_jade: 452c100875b3
  ev.lop.bazaar.title: 0a623c9c84bf
  ev.lop.caravanserai.body: eec6756e9392
  ev.lop.caravanserai.choice.hire_guide: 15bc568785eb
  ev.lop.caravanserai.choice.provision: 221950c48b9e
  ev.lop.caravanserai.choice.wait_caravan: 437d93bd2bee
  ev.lop.caravanserai.title: 6744e91fd206
  ev.lop.entry.body: 8bfb406cf93f
  ev.lop.shrine.body: bf6fffb88948
  ev.lop.shrine.choice.offer: 2e7c07820489
  ev.lop.shrine.choice.watch: 57e1354d9104
  ev.lop.shrine.title: 411cd7fe9473
  ev.ev_lop_entry.choice_1_result: 31e2a31d8542
  ev.ev_lop_entry.choice_2_result: 5a612d26bf51
  ev.ev_lop_bazaar.choice_1_result: cbdbeee893e2
  ev.ev_lop_bazaar.choice_2_result: 07b8543c07f9
  ev.ev_lop_caravanserai.choice_1_result: d41057c9090f
  ev.ev_lop_caravanserai.choice_2_result: 7f51984628d6
  ev.ev_lop_caravanserai.choice_3_result: 824b33e754c8
  ev.ev_lop_shrine.choice_1_result: 3ba95416d6a0
  ev.ev_lop_shrine.choice_2_result: fc65eac579b0
  ev.ev_lop_consequence_a.title: eb062c7d2ca6
  ev.ev_lop_consequence_a.body: 7af3c98eac27
  ev.ev_lop_consequence_a.choice_1: a388e02d95ab
  ev.ev_lop_consequence_a.choice_1_result: d58e45732a6d
  ev.ev_lop_consequence_a.choice_2: 4c3e5cb2d921
  ev.ev_lop_consequence_a.choice_2_result: 904c68945651
  ev.ev_lop_consequence_a_resolution.title: 8da8411c8076
  ev.ev_lop_consequence_a_resolution.body: 8f02a293460b
  ev.ev_lop_consequence_a_resolution.choice_1: 9026575c45a6
  ev.ev_lop_consequence_a_resolution.choice_1_result: e5f80e3b0abf
  ev.ev_lop_consequence_a_resolution.choice_2: 906bc885b2c6
  ev.ev_lop_consequence_a_resolution.choice_2_result: 6a301c26d2b7
  ev.ev_lop_consequence_b.title: af7be3ab3455
  ev.ev_lop_consequence_b.body: fdfbd0596466
  ev.ev_lop_consequence_b.choice_1: e7ea14f7220c
  ev.ev_lop_consequence_b.choice_1_result: 1d7c341e2546
  ev.ev_lop_consequence_b.choice_2: 89ba37c166c9
  ev.ev_lop_consequence_b.choice_2_result: 764d32532634
  ev.ev_lop_consequence_b_resolution.title: 1e68cb6e4816
  ev.ev_lop_consequence_b_resolution.body: 57174b4257ea
  ev.ev_lop_consequence_b_resolution.choice_1: f90772d45222
  ev.ev_lop_consequence_b_resolution.choice_1_result: 21f57136f682
  ev.ev_lop_consequence_b_resolution.choice_2: 8dc5da77ae0f
  ev.ev_lop_consequence_b_resolution.choice_2_result: bdd77d33624b
  ev.lop.mentor_closure.title: da4a56320d6c
  ev.lop.mentor_closure.body: 7c7b8735668f
  ev.lop.mentor_closure.choice_1: da8d611ab3c7
  ev.lop.mentor_closure.choice_1_result: 7247e877d5a4
  ev.lop.mentor_closure.choice_2: 59a447ce21bf
  ev.lop.mentor_closure.choice_2_result: 1c9ce8b2614e
  ev.lop.mentor_closure.choice_3: 630a8fcfe069
  ev.lop.mentor_closure.choice_3_result: 1eb7e05498bf
  ev.ev_lop_bazaar_followup.title: 5397ba750946
  ev.ev_lop_bazaar_followup.body: c07b8528feb6
  ev.ev_lop_bazaar_followup.choice_1: e8539bc57bc4
  ev.ev_lop_bazaar_followup.choice_1_result: fbfc9d281bc9
  ev.ev_lop_bazaar_followup.choice_2: cdff509562a7
  ev.ev_lop_bazaar_followup.choice_2_result: 06f934bcc0e3
  ev.ev_lop_bazaar_followup.choice_3: 223789c7410b
  ev.ev_lop_bazaar_followup.choice_3_result: 89a34afa9674
  ev.ev_lop_caravanserai_followup.title: 2c5ce3d0124f
  ev.ev_lop_caravanserai_followup.body: 74d89fe86a52
  ev.ev_lop_caravanserai_followup.choice_1: 26c3cac4361b
  ev.ev_lop_caravanserai_followup.choice_1_result: 67afcc8eafc3
  ev.ev_lop_caravanserai_followup.choice_2: 76c5ca7667cc
  ev.ev_lop_caravanserai_followup.choice_2_result: 4c2fd1649f7d
  ev.ev_lop_caravanserai_followup.choice_3: 779c9dfc3734
  ev.ev_lop_caravanserai_followup.choice_3_result: 6afa57948ff5
  ev.ev_lop_shrine_followup.title: 765ebdfbd010
  ev.ev_lop_shrine_followup.body: 976ef3ffe69d
  ev.ev_lop_shrine_followup.choice_1: 082b57c032d1
  ev.ev_lop_shrine_followup.choice_1_result: 41614c2f7ce0
  ev.ev_lop_shrine_followup.choice_2: 048f68e19400
  ev.ev_lop_shrine_followup.choice_2_result: 958d9fbbea42
  ev.ev_lop_shrine_followup.choice_3: 89c63abdbb14
  ev.ev_lop_shrine_followup.choice_3_result: b0b99f0a710e
  ev.ev_lop_mentor_followup.title: 5332c05794cc
  ev.ev_lop_mentor_followup.body: 4743bb6c62ce
  ev.ev_lop_mentor_followup.choice_1: 8cb26b1d1e3c
  ev.ev_lop_mentor_followup.choice_1_result: ba3cd8db62cc
  ev.ev_lop_mentor_followup.choice_2: 537fd856974c
  ev.ev_lop_mentor_followup.choice_2_result: 9dd2b93b3a17
  ev.ev_lop_mentor_followup.choice_3: 06628fbab7e0
  ev.ev_lop_mentor_followup.choice_3_result: 834f677ea73a
  codex.cx-desert-of-lop.body: 274170ec9ede
  codex.cx-jade-rivers.body: 5a1ebccdd235
  codex.cx-desert-voices.body: 268c27bb3a94
---

## city.lop.name

罗卜

## ev.lop.entry.choice.ask

打听人们怎么说这片沙漠

## ev.lop.entry.choice.rest

在路边歇一日

## ev.lop.entry.title

罗卜城

## ev.lop.bazaar.body

罗卜的bāzār（集市）溢出城墙，摊到商队集结的砾石原上。于阗与沙州来的商人，在铺地的毯子上摊开货物：
成匹的丝绸、袋装的干瓜、压紧的茶砖，以及——最稀罕的——于阗上游河中所出之玉。玉商另坐一处，
把石头摆在黑绒上显色。一块拳头大的羊脂白玉，价过一峰骆驼。商人微笑着把它举向日光；
光透过去，如水一般。

## ev.lop.bazaar.choice.ask_prices

问这玉是从哪里来的

## ev.lop.bazaar.choice.buy_jade

买这块羊脂玉（400钱）

## ev.lop.bazaar.title

玉市

## ev.lop.caravanserai.body

罗卜的商队客栈是大漠之前最后一站——广阔院落，周围是马厩与睡台，满是骆驼、尘土与炊烟的气味。
刚走出沙漠的旅人呆坐荫下，喝着掺少许盐的水，补回黄沙夺去的东西。即将上路的人听着他们的故事，
畏惧渐深。店主提供向导、粮秣与劝告，收银子，找零用干肉。墙上涂满十数种文字的题记：
走过去的人留下姓名，走不过去的人留下祷词。

## ev.lop.caravanserai.choice.hire_guide

雇一名沙漠向导（200钱）

## ev.lop.caravanserai.choice.provision

备足粮秣（80钱）

## ev.lop.caravanserai.choice.wait_caravan

等候商队（八日，120钱）

## ev.lop.caravanserai.title

商队客栈

## ev.lop.entry.body

罗卜是个大镇，正处在罗布沙漠的边缘。凡是要横渡沙漠的旅人，都在这里歇上七日，让人马恢复过来。然后备足
一个月的人畜口粮，才继续向前走。你须知道：人们说这片沙漠极其漫长，骑马走一年也走不到尽头；就算是最窄的地方，
也仍要一个月。眼里全是黄沙和荒山，没有半点可吃的东西；可一昼夜之间，就能寻到甘甜的泉水，
足够五十人或一百人连同牲口饮用。

## ev.lop.shrine.body

城东低崖间，风凿出浅穴的迷宫。壁上满是数世纪旅人留下的炭画：骆驼、骑手、阿拉伯文与梵文的祷词、
姓名与年月。最大的洞里有一座小小的佛龛——泥塑佛身贴着廉价金箔，周围是燃尽的香与干果供品。
僧人早已不在，守龛的是一个面如皮革的唐古忒老妇，照料灯火，收下旅人留下的钱。

## ev.lop.shrine.choice.offer

留下一份供奉（50钱）

## ev.lop.shrine.choice.watch

看香客们如何行事

## ev.lop.shrine.title

罗卜石窟

## ev.ev_lop_entry.choice_1_result

你在沙漠边缘的路边歇了半日，给牲口饮水。一位熟知罗卜路上每一处水源的老守井人望着你，走过来搭话。

## ev.ev_lop_entry.choice_2_result

这片荒原前最后一个镇子的人，郑重地回答了你的问题。他们对穿越之路的说法并不一致，其中一人说，日落前会把真实情况带来给你。

## ev.ev_lop_bazaar.choice_1_result

商贩把羊脂玉托在手心掂了掂才肯放手，价钱就此说定。但这块玉并不是交易的全部——他按住你的手片刻，凑近了身子。

## ev.ev_lop_bazaar.choice_2_result

商人们告诉你，玉石自东边的河流而来。沙州之名如今上了你的舆图，玉河的流向也写进了你的行纪。

## ev.ev_lop_caravanserai.choice_1_result

向导收下你的两百枚钱，数了两遍，点了下头。他话不多，却用看自家马匹的眼神打量了你的牲口。

## ev.ev_lop_caravanserai.choice_2_result

你在驿栈中等了八日，向店主付了食宿之资。待商队终于启程，通往沙州的路已在眼前展开。

## ev.ev_lop_caravanserai.choice_3_result

你为干渴的路程备足了干粮，并把沙漠的规矩记进行纪：一日之内无食无水，亦无人烟。沙漠能拿走的，它必会拿走。

## ev.ev_lop_shrine.choice_1_result

你把供物放进小庙的佛龛，退后一步。看守洞窟的僧人走近前来——不为那枚钱，而是想讲讲你之前的旅人留下了什么。

## ev.ev_lop_shrine.choice_2_result

你默默看着朝圣者献上供奉。沙漠圣祠的礼俗，如今写进了你的行纪。

## ev.ev_lop_consequence_a.title

罗卜：门上的印记

## ev.ev_lop_consequence_a.body

第一个回答并没有在城门处结束。一位熟知罗卜路上每一处水源的老守井人把你引到一旁，问你打算从罗卜带走什么。问题背后正是沙海与旧井；他提出说出沙海那一头一口井的名字，说它至今仍甘甜。你须决定如何对待这道征兆。

## ev.ev_lop_consequence_a.choice_1

收下这个名字，追问他有何凭据

## ev.ev_lop_consequence_a.choice_1_result

你收下守井人递来的名字，追问他有何凭据。他取出一片那口井沿上敲下的石片，此事又打开了。

## ev.ev_lop_consequence_a.choice_2

谢过老守井人，把自己的打算收在心里

## ev.ev_lop_consequence_a.choice_2_result

你谢过老守井人，把自己的打算收在心里。此事在井边就此了结，罗卜不再有人向你多问一句。

## ev.ev_lop_consequence_a_resolution.title

罗卜：守井人的印记

## ev.ev_lop_consequence_a_resolution.body

你已答应把守井人的印记带在身上——一个他担保能打开沙海那一头井口的名字。它不会替你决定整段旅程，但在无水可饮的日子里，它或许正是咸与甜的分界。

## ev.ev_lop_consequence_a_resolution.choice_1

在天黑前把这个名字定下

## ev.ev_lop_consequence_a_resolution.choice_1_result

你在天黑前把这个名字定了下来。守井人的承诺如今随你的名字而行，沙漠里的人从此有理由接待你。

## ev.ev_lop_consequence_a_resolution.choice_2

把守井人的名字收进自己的行纪

## ev.ev_lop_consequence_a_resolution.choice_2_result

你把守井人的名字收进行纪，花了一日把它工工整整记下。记录已成；那分人情便等着一条或许永远不会叫它的路。

## ev.ev_lop_consequence_b.title

罗卜：回答之后的路

## ev.ev_lop_consequence_b.body

回答把一条传闻送上了道路。日落之前，一位商队厨子找到你，讲起沙海与旧井的另一番说法——哪些井至今有水，哪些已经咸了。这说法不如游记宏大，却更适合带在身上。

## ev.ev_lop_consequence_b.choice_1

照着这位厨子的实用建议走

## ev.ev_lop_consequence_b.choice_1_result

你照着这位厨子的实用建议走，请他把它写下来。他为你把这段叙述抄出，沙漠真正的尺度便落进你手里。

## ev.ev_lop_consequence_b.choice_2

把这段叙述留给那位厨子

## ev.ev_lop_consequence_b.choice_2_result

你把这段叙述留给那位厨子，自己收住了口。它回到了井边，罗卜不再向你索取什么。

## ev.ev_lop_consequence_b_resolution.title

罗卜：甜井的账目

## ev.ev_lop_consequence_b_resolution.body

你已选择带着这位厨子的账目上路——哪些井至今有水，哪些已经咸了。它不会替你决定整段旅程，但在无水可饮的日子里，它是一张舆图也画不出的地图。

## ev.ev_lop_consequence_b_resolution.choice_1

在天黑前把甜井标出来

## ev.ev_lop_consequence_b_resolution.choice_1_result

你在天黑前把甜井标上了舆图。沙漠的尺度从此走在你前头，它落在何处，你的名字便随到何处。

## ev.ev_lop_consequence_b_resolution.choice_2

把这份井位图收进自己的行纪

## ev.ev_lop_consequence_b_resolution.choice_2_result

你花了一日，把这份井位图抄进行纪。这份记录从此是你的了，只是为此在路上耗去一日。

## ev.lop.mentor_closure.title

罗卜的引路人

## ev.lop.mentor_closure.body

离开罗卜之前，沙海边的听风人问你从这里看出了什么。这个教诲不是法术，也不是交易；它只是教你留意道路转弯之后真正有用的细节。

## ev.lop.mentor_closure.choice_1

问这条路将要求你付出什么

## ev.lop.mentor_closure.choice_1_result

听风人并不回避：水与观天之明，都会在路上向你索取。他把这番功课递到你面前，如何处置全在于你。

## ev.lop.mentor_closure.choice_2

留下 一日，听完这段完整的讲述

## ev.lop.mentor_closure.choice_2_result

你在读风人身边留了一日，他把全部的忠告从容地讲给你。那不是一张地图，却会替你读脚下的沙。

## ev.lop.mentor_closure.choice_3

谢过引路人，继续上路

## ev.lop.mentor_closure.choice_3_result

你谢过听风人，踏上了路。他放你前行，为你祝福；沙漠的这番功课，便没有落笔。

## ev.ev_lop_bazaar_followup.title

罗卜：玉商的盐泽之言

## ev.ev_lop_bazaar_followup.body

罗卜的bāzār（集市）散场之后，最后一峰骆驼也被牵走，玉商折起他的黑绒，招手唤你到毯子尽头。他看过你掂量他的石头，知道你不是那种能骗第二次的买主。他压低声音，告诉你玉真正的来处：于阗上游的河床，雪山的溪流奔下，秋水一落，卵石便裸露出来。他说最好的石头都在水浅的季节捡拾，懂得时令的人，在源头用市价的一成便能买到。随后他又说起罗布沙漠之外的盐泽——一片白色的国土，脚下像冻雪一样咔嚓作响，寸草不生，旧井相距甚远，有的甘甜，有的苦涩到牲口不肯饮。他说去沙州的路正从那最窄处穿过，商队怕盐泽更甚于怕沙，因为在能要人命的水边百步之内，人照样会渴死。这番话，这个季节他还没有对别的买主讲过。他在等，看你拿它如何。

## ev.ev_lop_bazaar_followup.choice_1

买下玉商私藏的白玉

## ev.ev_lop_bazaar_followup.choice_1_result

玉商从腰带间的袋子里取出一块更小的石头，报出低于市价的价格，把它举到光下，让你看见玉质的油润。你离开集市时，带着别的买主从未见过的玉。

## ev.ev_lop_bazaar_followup.choice_2

问河川何时献出美玉

## ev.ev_lop_bazaar_followup.choice_2_result

玉商在尘土里替你画出于阗上游的河川，又标出向东通往沙州的路。那名字如今上了你的舆图，你也知道了石头现身的季节。

## ev.ev_lop_bazaar_followup.choice_3

留下听一听盐泽的传说

## ev.ev_lop_bazaar_followup.choice_3_result

你坐在毯子上看日头落下，玉商讲起白色的盐泽，和立在沙与路之间的旧井。耗去一日，而那讲述从此随身。

## ev.ev_lop_caravanserai_followup.title

罗卜：向导的分程

## ev.ev_lop_caravanserai_followup.body

沙漠向导在罗卜的商队客栈最后一堆火熄灭之前找到了你。他往来罗布沙漠，十九日达沙州，次数多得自己也数不清；他带了一根棍子，好把路画在尘土里。他并不先讲水。他先讲盐。他说这段路程其实是两片沙漠：前几日是沙，之后是盐泽，地面白而龟裂，空气里带着金属的味道。夏季须趁夜赶路，白日卧息，因为正午的沙能烫穿骆驼的蹄。旧井照旧例隔一日一夜一处；有的甘甜，有的发咸，有一处，他说，还没看见先闻见。他又说起那座干石垒成的塔，商队在那里留下记号与祷词；还有让沙丘唱歌的风，旅人循声走去，以为听见人语。讲完了，他用脚跟把尘土抹平，望着你，等你回答。

## ev.ev_lop_caravanserai_followup.choice_1

答应向导，待风转时一同上路

## ev.ev_lop_caravanserai_followup.choice_1_result

向导在大拇指上记下日子，叫你黎明前备妥。去沙州的路从此对你敞开，画在尘土里，也画在你心里。

## ev.ev_lop_caravanserai_followup.choice_2

照向导的单子备齐过漠之粮

## ev.ev_lop_caravanserai_followup.choice_2_result

你照他的单子采买：干肉、硬饼、盐，和每头牲口一只水囊。粮秣备齐，你也记住了能载你过漠的规矩。

## ev.ev_lop_caravanserai_followup.choice_3

留一日，学这段路程的规矩

## ev.ev_lop_caravanserai_followup.choice_3_result

你在商队客栈里留了一日，听向导复述各程：旧井、荒山、唱歌的沙丘。等待耗去一日，路程因此分明。

## ev.ev_lop_shrine_followup.title

罗卜：僧人的记述

## ev.ev_lop_shrine_followup.body

黄昏时分，罗卜石窟的守龛人走到你身边坐下，灯盏的小光落在泥佛身上。她便是那个照料灯火、收下钱币的老妇；僧人，你听人说已经走了。她并不否认。但她告诉你他们去了哪里，留下了什么：他们本是沙路之僧，守着罗卜与沙州之间的旧井，为商队标记路程。后来井水渐涸，盐泽漫上旧牧场，僧人们便东去，带走了横渡的记载。带不走的，他们刻在洞壁上——不只是祷词，还有井与井之间的日数、荒山的形状、对坏水的告诫。她指向洞角一处炭迹已经褪成岩石颜色的地方，说这旧记仍然可信，虽然执笔的手已经不在了；抄下它的旅人，不会在盐泽中迷失。说罢她又添了灯油，把你留给佛像与寂静。

## ev.ev_lop_shrine_followup.choice_1

请守龛人读出僧人的记载

## ev.ev_lop_shrine_followup.choice_1_result

守龛人像读一封信那样读着洞壁，一一说出每口井与每句告诫。僧人的记载从此归你，盐泽于你不再有秘密。

## ev.ev_lop_shrine_followup.choice_2

为僧人的灯献上银钱

## ev.ev_lop_shrine_followup.choice_2_result

你在供品间放下银钱，守龛人只点点头，不言不语。灯会多亮一些时候，沙路的祝福落在你身上。

## ev.ev_lop_shrine_followup.choice_3

留一日，把记载抄下洞壁

## ev.ev_lop_shrine_followup.choice_3_result

你在洞壁前过了一日，把褪色的炭迹抄进行纪。耗去一日，而这份记载从此写在你路上可读之处。

## ev.ev_lop_mentor_followup.title

罗卜：听风人的教诲

## ev.ev_lop_mentor_followup.body

读风之人站在城边，砾石原尽头，与你并肩向东望去。他一辈子读风，不把沙漠当作可畏之物，而当作可读之书。他告诉你，沙自己会写字：沙丘的纹理说出刮过什么风，干裂的盐壳说出旧海退去多远，空气的味道说出前方是水，还是只有水的记忆。他教你夜渡认星之法，教你去听唱歌的沙丘——那不是人语，是沙在自言自语。他的告诫不是符咒；是一种留神的法子。他把它给你，因为你问错了时候却问对了问题，而那是学东西最好的时候。说完他退后一步，把路留给你。

## ev.ev_lop_mentor_followup.choice_1

把听风人的告诫带在路上

## ev.ev_lop_mentor_followup.choice_1_result

听风人的告诫写进了你的行纪，沙漠从此读起来两样。他教给你的，在沙起之处不会辜负你。

## ev.ev_lop_mentor_followup.choice_2

问沙漠将向你索取什么

## ev.ev_lop_mentor_followup.choice_2_result

听风人为这段路程读了征兆，告诉你路要什么：水、耐性，还有趁夜行路的决心。这警告从此随身。

## ev.ev_lop_mentor_followup.choice_3

谢过听风人，循他指的路走去

## ev.ev_lop_mentor_followup.choice_3_result

你谢过听风人，顺他标出的路向东行。去沙州的路已在眼前展开，你把他那句未说出口的告诫藏在心里。

## codex.cx-desert-of-lop.body

罗布沙漠在罗卜之东，人们说极长——骑马一年也走不到尽头，最窄处仍须一月。渡漠者歇足七日，
备足一月之粮；旧井照旧例隔一日一夜一处，甘泉一昼夜可寻，足供五十人或百人连同牲口饮用。
向导教我夏月趁夜赶路、白日卧息。沙路驿栈之僧曾守罗卜与沙州之间的旧井，为商队标程；井涸东去前，
他们把井与井之间的日数、荒山形状与坏水告诫刻在洞壁——我抄下时，炭迹已褪成岩色。

## codex.cx-jade-rivers.body

玉出何处？罗卜的玉商说，在于阗上游的河床——雪山的溪流奔下，秋水一落，卵石便裸露出来；最好的石头都在
水浅的季节捡拾，懂得时令的人在源头用市价的一成便能买到。集市上他们把玉摆在黑绒上显色：一块拳头大的
羊脂白玉，价过一峰骆驼。商人举它向日光，光透过去，如水一般。玉河的流向，如今写进了我的行纪。

## codex.cx-desert-voices.body

罗卜乃大漠边缘之大镇，此漠名罗布沙漠。人言夜渡此漠，若有人落后或睡沉，欲追大队时，便闻空中精灵语，
声似同伴；有时精灵直呼其名，旅人常被诱离，终不得复见队伍，以此死者甚众。即白昼，人亦闻此精灵之声，
有时空中尚有鼓、笛之乐器声。此漠之险如此——我离开罗卜时，把这番告诫写进了行纪。
