...
GDD §16 的 MVP 十二城里泉州与大都在框外；马可·波罗语料 136 个 places 里 **52 个在中国**（`LORE_PIPELINE.md §1`）。**不东扩就等于丢掉内容最厚的那一半。**
...
| 问题 | 措施 |
|---|---|
| iOS 包体与 OTA 下载限制 | 城市包按需加载；核心包只装 MVP 12 城 |
...
| # | 风险 | 影响 | 应对 |
|---|---|---|---|
| R1 | **worldmap 东扩未做** | MVP 城市有一半在框外 | §4.1，**开工第一件事** |
| R2 | **MVP 十二城与语料不匹配** | 5/12 城零语料 | `LORE_PIPELINE.md §2` 已定采用方案 A（中国—中亚走廊），但 **GDD §16 尚未同步改写**，必须先改 |
...
1. **改 GDD §16**：按 `LORE_PIPELINE.md` 方案 A 重排 MVP 十二城（R2）——不改这个，后面填的表都是废的
2. **worldmap 东扩**（R1）：改脚本、装 `shapely/rasterio`、重跑四个生成器
...
6. **M1 验收**（GDD §17）：八张表落库、12 城、三条线可从头走到尾
