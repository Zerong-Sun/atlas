...
| 语料背书 | 83/102 城有 `origin: "source"`（波罗 63 + 三书 20），19 城 `authored`（T6 已查标注） |
| 文本 | en/zh 各 2482 条，缺 0 |
| 测试 | 15 单测 + 10 smoke，全过 |
| 门禁 | G1–G3、G7–G18、G20–G27 全绿（G27 steppe WARN 预期） |

**一句话**：机器都通了；T3/T4/T6 密度任务已闭环——12 座主城各 3 点、21 座 city 各 2 点、草原途中 48 条。
...
| **T3** | 次级城探索点 | 21 座 city 各 2 点 | — | §4 ✅ 2026-07-26 |
| **T4** | 法德兰草原 stories 转途中事件 | 约 40 条 | — | §5 ✅ 2026-07-26 |
| **T5** | 补 F-3 / F-6 两项审计遗留 | 小 | 无 | §6 ✅ 2026-07-26 |
| **T6** | 11 条弱证据复核 + 8 条查无处置 | 19 城 | — | §7 ✅ 2026-07-26 |
...
| **G26** | `city` 级城市探索点数量（2）与 metropolis（3）分级正确 | ✅ T3 | 已验：给某 city 挂 3 点 → 报错 |
| **G27** | 途中事件的 band 分布不得过度倾斜（任一 band ≥ 50% 即警告） | ✅ T4 | 已验：steppe ≈59% → WARN |
