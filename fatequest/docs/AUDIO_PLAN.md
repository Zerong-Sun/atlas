...
|---|---|
| 音频资产 | **0 个文件** |
| 旧版实现 | `js/audio.js` 248 行 WebAudio 程序化（`tone`/`hiss`/`bell`/`PADS`/`startDrone`），**思路可借鉴，代码作废**（D3） |
| 包体压力 | `assets/` 已 179 MB，iOS 分发是硬约束（`ARCHITECTURE.md` §6） |
...
| 禁止 | 理由 |
|---|---|
| 合成或模仿**宣礼**（adhān）、**诵经**（Qur'ān recitation） | 是活信仰的礼拜行为，不是配乐素材 |
| 合成或模仿佛道**唱诵**、基督教**礼仪圣咏**的具体曲目 | 同上 |
| 把任一传统的调式当作「异域风味贴纸」 | GDD P3：文化差异要落在机制上，不是换皮 |
| 圣所场景使用戏谑、诡异或悬疑音色 | GDD P4：对活着的信仰保持敬畏 |

**允许且应当**：使用各地**世俗**器乐的音色与调式作为**指向**（gesture），并在图鉴中说明这是演绎而非记录。

> **调式选择是一种致意，不是一种代表。** 把 maqām 或 rāga 简化成一个音阶，本身就是简化；文档里写清楚这一点，比假装忠实要诚实。

**发布门禁**：音频与美术、文本一并纳入 GDD §19 的上线前敏感读者审阅，**尤其是圣所场景**。
...
| 文化 | 调式指向 | 音色指向 | 速度 | 节拍感 |
|---|---|---|---|---|
| `islamic` | Hijaz / Bayati 的音程色彩（增二度） | 弹拨（乌德类）· 竹笛类（奈伊类）· 框鼓 | 72–84 | 不规则长句，句尾拖 |
| `east_asia` | 五声（宫商角徵羽），偶用清角变宫 | 丝弦（古琴/琵琶类）· 横笛 · 编钟余韵 | 56–68 | 疏，留白多，散板感 |
| `steppe` | 五声 + 泛音列 | 马头琴类弓弦 · 泛音人声（**器乐化处理，不做具体唱诵**） · 低鼓 | 60–76 | 长音铺底 + 马蹄型节奏 |
| `indian_ocean` | 持续音上的旋律移动（rāga 之**结构**思路，不引用具体 rāga） | 持续音（tanpura 类）· 拨弦 · 手鼓 | 80–96 | 循环渐密 |
| `latin` | 中世纪教会调式（多利亚 / 混合利底亚） | 拨弦（诗琴类）· 擦弦（维埃尔类）· 管风琴远景 | 64–72 | 二部 organum 式平行 |
...
**圣所只留空间与偶发钟磬**，不加旋律层：这既是 §1 的尊重，也恰好是最省事的做法。
...
| 序 | 内容 | 验收 |
|---|---|---|
| A1 | 音频总线与静音／音量设置 | 有开关，默认可关 |
| A2 | 程序化装饰音（§7.3） | 点选项有反馈声 |
| A3 | 单文化单层 drone（先做 `east_asia`） | 进刺桐有持续声底 |
| A4 | 场景类密度切换（§4 的 6 类） | 沙漠夜明显更空 |
| A5 | 五文化全层 + 跨文化交叉淡入 | 跨 band 听得出变化 |
| A6 | 情绪调制（§5） | 高风险路段听感变紧 |
...
- **不做**每城独立配乐（102 城 × 曲 = 范围灾难；文化 × 场景 × 情绪的组合已足够区分）
- **不做**真实礼拜声响的合成或采样（§1 红线）
