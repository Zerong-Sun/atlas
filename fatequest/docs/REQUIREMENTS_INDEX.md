...
- 102 城、204 路线、332+ 事件、60 商品、54 随从、24 种占法注册；12 主城已接入后果链与 site/mentor 多轮互动；书案 `books` 表 8 册可读（G30）。
- 运行时美术 674 张，`art_wire_index.json` 为 674/674，未接线 0。
- en/zh 双语齐全；故事单元含 `books` / `codex_legends`；译文 stamps current。
- 语料：马可·波罗／三伊本已接入；瀛涯胜览／鲁布鲁克／鄂多立克／康蒂／塔富尔／中文行纪／曼德维尔已入库（见 `STATUS.md` §3、`SOURCE_NOTES.md`）。
