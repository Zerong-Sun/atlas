...
P3  能占卜          占法注册表 + 8 种引擎 + 影响路线      ✅ 2026-07（见 §5.2）
...
| 层 | 数 | 说明 |
|---|---|---|
| 真实引擎 + 接入玩法 | **8** | `iching` `bazi` `lot` `tarot` `jiaobei` `astrodice` `geomancy` `runes`——各有 `learnAt`、拜师事件、结果文本 |
| 通用引擎 `SoftDivinationMethod` | **16** | `cast` 出结构化随机、`to_effects` 只发一条 codex；尚未排期接入玩法 |

> **「注册了」仍不等于「玩得到」。** 其余 16 种是有意保留的扩展接口（`mvp:
> false`），不计入当前可玩规模。8 种 MVP 占法均由 G25 与
> `test_divination_reach` 保证可学、可用。

第二批 `jiaobei` `astrodice` `geomancy` `runes` 已于 2026-07-26 完成。实现、
测试与门禁记录见 `PLAN.md` §3。
...
- [x] **第二批 4 法接入玩法**（见 `PLAN.md` §3.6）
- [x] `test_divination_reach`：每个 `mvp: true` 的占法**可学、可用、端到端跑通**
- [x] 门禁 G25 上线并完成负向测试
...
## 11. 立即要做的三件事（2026-07-26）

P0–P6 七个阶段全部完成，玩法已闭环。当前三件按 `PLAN.md` 展开：

1. **21 座 city 级探索点各 2 个**（`PLAN.md` §4）——世界密度目前是最短的板
2. **法德兰草原 stories 转约 40 条途中事件**（`PLAN.md` §5）——补强波罗线草原段
3. **19 城语料证据处置**（`PLAN.md` §7）——逐城复核弱证据，查无者维持新撰标签
...
| 24 种占法 | 全部注册、API 可调，但当时 20 种没有 `learnAt`，玩家学不到 | 已加 `test_divination_reach` + G25；现有 8 法可玩 |
