...
**2026-08-10 复核 · Steam EA 阶段。** 这是唯一的「现在到哪一步」权威页；`PLAN.md` 讲下一步，`ROADMAP.md` 讲阶段顺序。三份不重复。
...
| 表 | 条数 | 状态 |
|---|---|---|
| cities | 103 | ✅ 12 主城六件全套（2026-08-11 +马儿代夫） |
| routes | 206 | ✅ 三条线实测可走完（+马儿代夫 2 海路） |
| events | 429 | ✅ 含十二主城后果链、site/mentor 多轮、**21 city 深化 24 followup**、白图泰 13 探索事件 · 0 桩 |
...
| 地图 | ✅ OpenStreetMap 真实瓦片、Web Mercator、7 天缓存与离线矢量回退；鼠标/触控板/触屏连续平移缩放 |
...
真实地图六维验证与修复记录：[`REAL_MAP_VERIFICATION_2026-08-10.md`](REAL_MAP_VERIFICATION_2026-08-10.md)。
...
| OpenStreetMap 在线真实瓦片底图 + 7 天缓存 + `fatequest-worldmap` 离线矢量回退 | ✅ |
| 已知城市点击详情卡 | ✅ |
| 交通方式 → 目的地简介 → 确定出发 → 动画 | ✅ |
...
| 伊本·白图泰 | 1325–1354 | 46 | 18 | ✅ 已接入（10 城 lore + 13 探索事件 + 书案） |
...
**已兑现**：城市 lore 覆盖 **84/103** 主引证不变；另有 **31** 城挂 `altRefs` 二次声部（2026-08-11 白图泰 17 城，其余 14）。11 座弱证据 + 8 座查无保持 `origin: "authored"`。
...
```
en 6469 条 · zh 6469 条 · 缺 0 · 中英同文 0
...
| 103 城分支深化 | 12 主城 + **21 city 已多轮**；town/station 与其余 site 保持可运行（白图泰 13 探索事件为 town/station 层补充） | 需求书 §4.7、§10；后续按城市批次人工通读 |
