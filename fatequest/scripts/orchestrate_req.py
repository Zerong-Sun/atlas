...
def style_for_window(window: str) -> str:
    ...
    if w in ("desk",):
        return DESK_STYLE
    if w in ("ui", "lot", "rank", "cargo", "rose"):
        return UI_STYLE
    if w.startswith("cities") or w.startswith("band"):
        return SCENE_STYLE
    return MAP_STYLE
...
def load_pending(
    ...
    if window_order is None:
        ...
        if "P4" in upper or "P5" in upper or "P4P5" in upper:
        ...
        while True:
            ...
            while len(active) < args.max_windows and queue:
                ...
                try:
                    ...
                    print(f"  [{lane.name}] open failed: {e}", flush=True)
                    queue.insert(0, lane)
                    break
