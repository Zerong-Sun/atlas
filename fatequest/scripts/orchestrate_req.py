...
P2C_WINDOW_ORDER = ["Currency", "Sticker"]
...
def load_pending(
    ...
    *,
    ...
    if window_order is None:
        ...
        elif "P2_CONTENT" in upper or "P2C" in upper:
            window_order = P2C_WINDOW_ORDER
        elif "P2" in upper:
...
def status_path_for(prompts_file: str) -> Path:
    stem = Path(prompts_file).stem.replace(" ", "_")
    return Path(f"/tmp/orchestrate_req_status_{stem}.json")


def load_saved_chats(prompts_file: str) -> dict[str, str]:
    path = status_path_for(prompts_file)
    if not path.exists():
        return {}
    try:
...
async def run(args: argparse.Namespace) -> int:
    ...
    saved_chats = load_saved_chats(args.prompts_file)
    for lane in lanes:
    ...
    try:
        while True:
            ...
            summarize(lanes)
            status_path_for(args.prompts_file).write_text(
                json.dumps(
                    {
                        "round": round_i,
